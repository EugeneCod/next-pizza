...
const buttonVariants = cva(
  'inline-flex items-center justify-center whitespace-nowrap rounded-md active:translate-y-[1px] text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 aria-disabled:bg-gray-500',
  {
    variants: {
      variant: {
  ...
  ({ className, variant, size, asChild = false, children, disabled, loading, ...props }, ref) => {
    ...
    return (
      <Comp
        ...
        ref={ref}
        {...props}>
        {!loading ? children : <LoaderIcon className="w-5 h-5 animate-spin" />}
      </Comp>
    );
  },
);
